        ...
        if isinstance(top_msg, DealDamageMsg):
            ...
            for idx, (target_id, target_pos, element_type, dmg_val) in enumerate(top_msg.targets):
                if target_id == self.player_id and target_pos ==  self.position:
                    # print("From: ",top_msg.attacker)
                    # print("TO: ",target_id, target_pos, element_type, dmg_val)
                    # print("Now: ",self.player_id,self.position)

                    if element_type in [ElementType.NONE, ElementType.PYRO,ElementType.CRYO]:
